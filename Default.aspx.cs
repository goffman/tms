...
public partial class suite_Default:System.Web.UI.Page {
    ...
    int StatusOnDell() {
        ...
        RSD.Parameters.AddWithValue("@Password",Cryptography.Cryptography.Md5Hash(txtloginpass.Text.Trim()));
    ...
    protected void Button1_Click(object sender,EventArgs e) {
        ...
        try {
            ...
            if (Convert.ToInt32(CUN.ExecuteScalar().ToString())==1) {
                ...
                try {
                    if (CUP.ExecuteScalar().ToString()==Cryptography.Cryptography.Md5Hash(txtloginpass.Text.Trim())) {
    ...
    private void SendExceptionToEmail(Exception ex) {
        SendEmail.SendToAdmin(ex.Message,"Ошибка в приложении");
    }
